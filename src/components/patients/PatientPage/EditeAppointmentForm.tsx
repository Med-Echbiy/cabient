...
function EditeAppointmentForm(props: props) {
  ...
  const handleAssetsDelete = async (assetId: string) => {
    ...
    try {
      ...
      const res = await deleteAsset(assets, assetId, props.event_id as string);
      if (!res) {
        ...
        <div className="inputs flex col-span-2 items-center gap-4">
          <div className=" capitalize flex-grow relative px-3">
            <Autocomplete
              ...
              options={getClients().map((e) => e.fullName)}
              renderInput={(params) => (
                <TextField
                ...
                {services.map((e, i) => (
                  <MenuItem key={e._id + i} value={e._id}>
                    <div className="flex capitalize items-center w-full gap-3">
                      ...
                      <div className="flex-grow flex items-center gap-3 justify-end text-sm">
...
export default EditeAppointmentForm;
